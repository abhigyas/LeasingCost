public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
      ...
      //getting due
        ...
        String d = description.substring(initialDues+4);
         String due = d.substring(0,d.indexOf(";"));
         double dueAtSigning = Double.parseDouble(due);
      //getting length
         int initialLength = description.indexOf("length:");
         String l = description.substring(initialLength+7);
         String length = l.substring(0,l.indexOf(";"));
         int leaseLength = Integer.parseInt(length);
      //getting monthly
        int initialMonth = description.indexOf("monthly:");
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
       //loop to parse through vehicles in array
        for(int i = 0; i < vehicles.length; i++){
       //checks what fuel type the vehicle has
            if(vehicles[i].getFuel().getType() == 1){
                vehicles[i].setCO2Emission(computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(),vehicles[i].getFuel().CO2EMITTED_GASCOMBUSTION));
                ...
                vehicles[i].setCO2Emission(computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(),vehicles[i].getFuel().CO2EMITTED_ELECTRICITYCOMBUSTION));
                ...
                vehicles[i].setTotalCost(computeLeaseCost(vehicles[i].getLease().getDueAtSigning(), vehicles[i].getLease().getLeaseLength(), vehicles[i].getLease().getMonthlyCost()) + vehicles[i].getFuel().getCharger() + vehicles[i].getFuelCost()); 
            }
        }
